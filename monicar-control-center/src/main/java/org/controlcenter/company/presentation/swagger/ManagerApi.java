...
public interface ManagerApi {
	@Operation(summary = "로그인 기능",
		description = "<h1>로그인 기능</h1>"
			...
			+ "9995 오류 코드 발생 시 <strong>'/refresh'로 요청</strong>하여 액세스토큰 재발급 받고 다시 요청하려던 엔드포인트로 재요청"
			+ "<h2>액세스토큰이 유효하고 리프레시토큰이 만료된 경우(9994)</h2>"
			+ "9994 오류 코드 발생 시 <strong>'/reissue'로 요청</strong>하여 리프레시토큰 재발급 받고 다시 요청하려던 엔드포인트로 재요청"
